...
static void emit_lookup_run(FILE* out, const FunctionList* funcs, u32 first, u32 end) {
    const FunctionRange* first_range = &funcs->ranges[first];
    ...
    fprintf(out, "        if (offset < 0x%08Xu && (offset & 3u) == 0u) {\n", span);
    fprintf(out, "            static const DolRecompFunction chunk_functions[] = {\n");
