...
void StaticRecompCore::RefreshRelSections()
{
  ...
  std::fill(m_chunk_host_call_state.begin(), m_chunk_host_call_state.end(), u8{0});
  m_active_rel_sections = std::move(discovered);
  std::vector<Memory::RelocationAlias> aliases;
  aliases.reserve(m_active_rel_sections.size());
  for (const ActiveRelSection& section : m_active_rel_sections)
    aliases.push_back({section.linked_start, section.runtime_start, section.size});
  m_system.GetMemory().SetRelocationAliases(std::move(aliases));
...
int StaticRecompCore::ChunkIndexOf(u32 address)
{
  ...
  const int direct_index = GetAddressLookupIndex(address);
  if (direct_index >= 0 &&
      direct_index < static_cast<int>(m_chunk_lookup_table.size()))
  {
    const int direct_chunk = m_chunk_lookup_table[direct_index];
...
bool StaticRecompCore::DispatchableAt(u32 address)
{
  ...
  if (!m_forced_fallback_ranges.empty() && IsForcedFallbackAddress(address))
    return false;
  const int index = ChunkIndexOf(address);
  if (index < 0 || ChunkContainsHostCall(static_cast<u32>(index)))
    return false;
  ++m_smc_probe_clock;
...
bool StaticRecompCore::ChunkContainsHostCall(u32 index) const
{
  if (!m_host_calls_active || !m_module_source.host_call_contains ||
      index >= m_chunk_host_call_state.size())
    return false;
  ...
  if (m_module_source.host_call_range_contains)
  {
    ...
    if (!found && runtime_start != chunk.start)
    {
      found = m_module_source.host_call_range_contains(
          runtime_start, runtime_start + (chunk.end - chunk.start),
...
void StaticRecompCore::RefreshHostCalls()
{
  const u64 generation = m_module_source.host_call_generation ?
                             m_module_source.host_call_generation(m_module_source.host_call_user) :
                             0;
  if (generation != m_host_call_generation)
  {
    m_host_call_generation = generation;
    std::fill(m_chunk_host_call_state.begin(), m_chunk_host_call_state.end(), u8{0});
  }

  m_host_calls_active = m_module_source.host_call &&
                        (m_module_source.host_call_active ?
                             m_module_source.host_call_active(m_module_source.host_call_user) :
                             true);
  m_guest.host_call = m_host_calls_active && m_module_source.host_call ? HookHostCall : nullptr;
}

bool StaticRecompCore::NativeRegionAvailable(u32 start, u32 end)
{
  RefreshHostCalls();
  if (!m_host_calls_active)
    return true;
  if (!m_module || start >= end)
    return false;

  const StaticRecompRange* begin = m_module->chunk_ranges;
  const StaticRecompRange* finish = begin + m_module->num_chunk_ranges;
  const StaticRecompRange* chunk = std::lower_bound(
      begin, finish, start,
      [](const StaticRecompRange& range, u32 address) { return range.end <= address; });
  if (chunk == finish || chunk->start >= end)
    return false;

  for (; chunk != finish && chunk->start < end; ++chunk)
  {
    if (ChunkContainsHostCall(static_cast<u32>(chunk - begin)))
      return false;
  }
  return true;
}
...
{
  const auto& chunk = m_module->chunk_ranges[index];
  const bool was_failed = m_chunk_state[index] == CHUNK_FAILED;
  ...
  {
    m_chunk_state[index] = CHUNK_VERIFIED;
    m_chunk_retry_after[index] = 0;
    if (was_failed && m_failed_chunks != 0)
    {
      --m_failed_chunks;
      std::fprintf(stderr, "[staticrecomp] SMC: chunk [0x%08X,0x%08X) restored; "
                           "native execution resumed\n",
                   chunk.start, chunk.end);
    }
  }
  else
